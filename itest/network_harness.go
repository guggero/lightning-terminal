...
func (n *NetworkHarness) SetUp(t *testing.T,
	...
	mockServerAddr := fmt.Sprintf(listenerFormat, NextAvailablePort())
...
func (n *NetworkHarness) NewNode(t *testing.T,
	...
	node, err := n.newNode(name, extraArgs, nil, false, nil, true, opts...)
...
func (n *NetworkHarness) NewNodeWithSeed(name string, extraArgs []string,
	...
	return n.newNodeWithSeed(
		name, extraArgs, password, nil, statelessInit,
	)
}

func (n *NetworkHarness) newNodeWithSeed(name string, extraArgs []string,
	password, entropy []byte, statelessInit bool) (
	*HarnessNode, []string, []byte, error) {

	node, err := n.newNode(name, extraArgs, nil, true, password, true)
	if err != nil {
...
func (n *NetworkHarness) RestoreNodeWithSeed(name string, extraArgs []string,
	...
	node, err := n.newNode(
		name, extraArgs, nil, true, password, true, opts...,
	)
	if err != nil {
		return nil, err
	}
