...
import { GrpcClient, LlmApi, LndApi, LoopApi } from 'api';
import {
  AccountStore,
  AuthStore,
  BuildSwapStore,
...
export const createStore = (grpcClient?: GrpcClient, appStorage?: AppStorage) => {
  ...
  const llmApi = new LlmApi(grpc);
  const csv = new CsvExporter();

  const store = new Store(lndApi, loopApi, llmApi, storage, csv, actionLog);
