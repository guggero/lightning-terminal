...
export default class AccountStore {
  ...
  /**
   * Closes an account via the LLM API
   */
  @action.bound
  async closeAccount(feeRate?: number) {
    try {
      ...
      const res = await this._store.api.llm.closeAccount(acct.traderKey, feeRate);
  ...
  /**
   * submits a withdraw of the specified amount to the LLM api
   */
  @action.bound
  async withdraw(amount: number, feeRate?: number) {
    try {
      ...
      const res = await this._store.api.llm.withdraw(acct.traderKey, amount, feeRate);
